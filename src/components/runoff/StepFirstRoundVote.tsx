...
// Candidatos reales de primera vuelta (se excluye el voto en blanco, que es
// una opción especial aparte).
const FIRST_ROUND_CANDIDATES = mockCandidates.filter((c) => c.id !== "c0")

const SPECIAL_OPTIONS = [
  { value: "blank", label: "Voté en blanco" },
  { value: "no_vote", label: "No voté" },
  { value: "na", label: "Prefiero no decir" },
]

interface StepFirstRoundVoteProps {
  value: string | null
  onChange: (v: string) => void
}

export function StepFirstRoundVote({ value, onChange }: StepFirstRoundVoteProps) {
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="font-display text-display-sm font-bold text-text">
          ¿Por quién votaste en{" "}
          <span className="text-primary">primera vuelta?</span>
        </h2>
      ...
      <div className="mx-auto max-w-sm space-y-2">
        ...
        <div
          ...
          role="radiogroup"
          aria-label="Voto en primera vuelta"
        >
          {FIRST_ROUND_CANDIDATES.map((c) => (
            <button
              key={c.id}
              type="button"
              role="radio"
